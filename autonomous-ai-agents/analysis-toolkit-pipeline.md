...
> 📍 此文档位于 `autonomous-ai-agents/` 根目录，是 prism、pressure-test、grill-me 三个 skill 的**共享流水线文档**。三个 skill 各自引用此文件，不通过 dispatcher 中转。
...
- 三个可以独立使用，也可以串联使用
- **prism-full 和 prism-reflect 分析后会将约束写入 `.prism-history.md`**，供后续 prism-scan 学习
- pressure-test 和 grill-me 的结果不持久化，仅在对话中展示
